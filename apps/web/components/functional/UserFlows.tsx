...
import { getSnapshotModeLabel, loadSnapshot, resetSnapshot, saveSnapshot } from "@amlbt/supabase";
...
function FlowShell({ children, logs, reset, source, syncError }: { children: ReactNode; logs: LogItem[]; reset?: () => void | Promise<void>; source?: string; syncError?: string }) {
  return (
    <div className="space-y-4">
      ...
      <Card className="border-blue-100 bg-amlbt-primary-soft/40">
        <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
          <div>
            <h3 className="font-semibold">Functional state</h3>
            <p className="text-sm text-amlbt-text-muted">Data source: <span className="font-semibold">{source ?? getSnapshotModeLabel()}</span>. Add Supabase env vars to persist every action in the database.</p>
            {syncError ? <p className="mt-1 text-sm text-red-700">Supabase sync fallback: {syncError}</p> : null}
          </div>
          {reset ? <Button variant="secondary" onClick={reset}>Reset state</Button> : null}
        </div>
...
export function OrdersFlow({ orderId, mode = "list" }: { orderId?: string; mode?: "list" | "detail" | "release" | "dispute" }) {
  ...
  const mutateOrder = (id: string, patch: Partial<AnyOrder>) => setState((prev) => ({ ...prev, orders: prev.orders.map((item) => item.id === id ? { ...item, ...patch } : item) }));
