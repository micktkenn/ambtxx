...
function AdminFlowShell({ children, logs, reset, source, syncError }: { children: ReactNode; logs: AdminLog[]; reset: () => void | Promise<void>; source?: string; syncError?: string }) {
  return <div className="space-y-4">{children}<Card className="border-blue-100 bg-amlbt-primary-soft/40"><div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center"><div><h3 className="font-semibold">Admin functional state</h3><p className="text-sm text-amlbt-text-muted">Data source: <span className="font-semibold">{source ?? getSnapshotModeLabel()}</span>. Admin actions persist to Supabase when env vars are configured.</p>{syncError ? <p className="mt-1 text-sm text-red-700">Supabase sync fallback: {syncError}</p> : null}</div><Button variant="secondary" onClick={reset}>Reset admin state</Button></div><div className="mt-3 grid gap-2">{logs.length ? logs.map((log) => <div key={log.id} className="rounded-xl border border-amlbt-border-soft bg-white p-2 text-sm"><Badge tone={log.tone}>{log.tone}</Badge><span className="ml-2">{log.message}</span></div>) : <p className="text-sm text-amlbt-text-muted">No admin action yet.</p>}</div></Card></div>;
}
